...
def get_model(input_shape):
    ...
    x = tf.signal.frame(inp, 128, 16, pad_end=True, axis=-2)
    x = keras.layers.BatchNormalization(-2)(x) # mel 기준으로 bn
    x = conv_block(x, 64)
    ...
    x = keras.layers.Bidirectional(keras.layers.GRU(36), merge_mode='sum')(x)
...
def get_wav_dataset(config, mode: str = 'train'):
    ...
    with ThreadPoolExecutor() as pool:
        x = np.stack(list(pool.map(open_wav, sorted(glob(path + '/*.wav')))), 0)
    x = x.reshape([x.shape[0], -1, x.shape[-2] // y.shape[-2], x.shape[-1]])
...
def main(config):
    ...
    # data load
    trainset = get_wav_dataset(config, 'train')
    valset = get_wav_dataset(config, 'val')
    testset = get_wav_dataset(config, 'test')
